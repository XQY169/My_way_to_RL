from utils import PPO_Actor, V_Critic,PPO_ReplayBuffer,PPO_Actor_lstm,V_Critic_lstm
from Abstraction_module_lstm import abstraction_module
import numpy as np
import copy
import torch
import math
...
class PPO_agent(object):
	def __init__(self, config,kwargs):
		...
		if self.abs:
			...
			self.actor = PPO_Actor_lstm(self.state_size1, self.action_size, self.neu_size).to(self.device)
			self.critic = V_Critic_lstm(self.state_size1, self.action_size, self.neu_size).to(self.device)
		...
		self.replay_buffer = PPO_ReplayBuffer(self.state_size1+self.action_size,self.his_length,self.action_size,self.max_steps,self.device)
		if self.abs:
			self.abs_module = abstraction_module(self.state_size1, self.state_size, self.abs_neu_size, self.action_size,self.abs_lr*(1e-5), self.device,self.scheduler)
	...
	def act(self, state, deterministic,with_logprob = True):
		with torch.no_grad():
			...
			if self.abs:
				state = self.abs_module.phi_net(state).squeeze(0)
			if deterministic:
	...
	def train(self):
		...
		# s,s_next = process_his_state(s,s_next,dw,his_length=32,device = self.device)
		# print(s.shape[0])
		if self.abs:
			...
			if self.train_abs >= 1:
				...
				self.train_iter += 1
				step_per_train = int(1/self.train_abs)
				if self.train_iter % step_per_train == 0:
					self.abs_module.train_with_data(s, a, r, s_next, batch_size=self.abs_train_size)
			with torch.no_grad():
