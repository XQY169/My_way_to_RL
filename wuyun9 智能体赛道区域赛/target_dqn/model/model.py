...
class Model(nn.Module):
    def __init__(self, state_shape, action_shape=0, softmax=False):
        ...
        fc_layer1 = [nn.Linear(np.prod(state_shape), 1024), nn.ReLU(inplace=True)]
        fc_layer2 = [nn.Linear(1024, 512), nn.ReLU(inplace=True)]
        self.fc_layerA = nn.Linear(512, np.prod(action_shape))
        self.fc_layerV = nn.Linear(512,1)
    ...
    def forward(self, s, state=None, info=None):
        ...
        logits_A = self.fc_layerA(logits)
        logits_V = self.fc_layerV(logits)
        logits = logits_V + logits_A - torch.mean(logits_A,1,True)
        return logits, state
