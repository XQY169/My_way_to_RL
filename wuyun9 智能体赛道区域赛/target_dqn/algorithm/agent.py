...
class Agent(BaseAgent):
    ...
    def __predict_detail(self, list_obs_data, exploit_flag=False):
        ...
        bump_act = [[row[i] for i in self.besides] for row in feature_map]
        bump_act = torch.tensor(np.array(bump_act))
        flag = False
        # memorys_grid = [row[9103] for row in feature_map]
        # flag = memorys_grid[0] >= 0.9
        if exploit_flag:
            memorys_grid = [row[9103] for row in feature_map]
            flag = memorys_grid[0] >= 0.9
        # print('bump_Act',bump_act)
        # print(flag)
        ...
        legal_act = (
            torch.cat(
                (
                    legal_act[:, 0].unsqueeze(1).expand(batch, self.direction_space)&bump_act,
    ...
    def learn(self, list_sample_data):
        ...
        _batch_obs_legal = torch.tensor(np.array([frame._obs_legal for frame in t_data]),dtype = torch.int32)
        ...
        _batch_bump_act = [[row[i] for i in self.besides] for row in _batch_feature_map]
        _batch_bump_act = torch.tensor(np.array(_batch_bump_act),dtype = torch.int32)
        _batch_obs_legal = (
            torch.cat(
                (
                    _batch_obs_legal[:, 0].unsqueeze(1).expand(batch, self.direction_space)&_batch_bump_act,
        ...
        with torch.no_grad():
            ...
            argmax_a = q.argmax(dim = 1).unsqueeze(-1)

        model = getattr(self, "target_model")
        model.eval()
        with torch.no_grad():
            q,h = model(_batch_feature,state = None)
            q_max = q.gather(1,argmax_a)
    ...
    def save_model(self, path=None, id="1"):
        ...
        # torch.save({'model_state_dict':self.model.state_dict()}, model_file_path)
