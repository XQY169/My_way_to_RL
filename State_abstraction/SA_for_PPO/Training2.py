        ...
        for epoch in range(train_param['max_epochs']):
            ...
            for i in range(train_param['steps']):
                if epoch < train_param['init_epochs']:
                    ...
                    a, logprob_a = self.agent.act(s, deterministic=False, with_logprob=True);
                    act = self.agent.process_action(a) * action_flag
                ...
                self.agent.replay_buffer.add(s,a,logprob_a,r,s_,done,dw,valid_env)
                s = s_;
