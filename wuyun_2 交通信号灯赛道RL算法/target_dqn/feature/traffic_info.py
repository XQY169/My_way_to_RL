...
class TrafficInfo:
    ...
    def __init__(self, logger):
        ...
        self.last_record_passing_vehicle = {}
        self.current_record_passing_vehicle = {}
        ...
        self.old_old_waiting_time = 0
        self.old_num_vehicle = 0
        self.old_record_time = 0

        self.old_decreasing_time1 = 0
        self.old_decreasing_time2 = 0
        self.old_decreasing_num = 0
        self.old_passing_time = 0
        self.old_obs = [0.0] * 840
    ...
    def cal_travel_distance(self, vehicle):#单位是mm
        """
    ...
    def get_all_junction_waiting_time(self, vehicles: list):
        ...
        for vehicle in vehicles:
            ...
            if vehicle.v_id in self.waiting_time_store:
                ...
                t = 0
            if vehicle.v_id in self.last_waiting_moment:
                w_num[vehicle.target_junction] += 1
    ...
    def get_all_delay_time(self,raw_obs,vehicles:list):
        frame_state = raw_obs[1].framestate
        ...
        for vehicle in vehicles:
            ...
            if vehicle.v_id in self.vehicle_distance_store:
                d_time[vehicle.target_junction] += (frame_time - self.enter_lane_time[vehicle.v_id])/1000.0 -(self.vehicle_distance_store[vehicle.v_id]*3.6/self.vehicle_configs_dict[vehicle.v_config_id].max_speed) 
                pass
        ...
        for junction in self.junction_dict:
            if v_num[junction] != 0:
                d_time[junction] /= v_num[junction]

        return d_time

    def record_waiting_vehicle(self):
    ...
    def calculate_decreasing_time(self):
        decreasing_time1 = 0.0
        decreasing_time2 = 0.0
        for v_id in self.last_record_waiting_vehicle.keys():
            if v_id in self.current_record_waiting_vehicle:
                decreasing_time2 += self.last_record_waiting_vehicle[v_id] - self.current_record_waiting_vehicle[v_id]
                # decreasing_time += - self.last_record_waiting_vehicle[v_id]
                # decreasing_time += 0.0
            else:
                decreasing_time1 += self.last_record_waiting_vehicle[v_id]
                # decreasing_time += 0
        
        # print(decreasing_time)
        return (decreasing_time1,decreasing_time2)

    # def calculating_anger_vehicle(self,in_line_vehicle):
    #     anger_vehicle = 0
    #     threashold = 8*1000
    #     for v_id in in_line_vehicle:
    #         if v_id in self.waiting_time_store:

    #             if self.waiting_time_store[v_id] >= threashold:
    #                 anger_vehicle += 1
        
    #     return anger_vehicle


    def record_passing_vehicle(self):
        self.last_record_passing_vehicle = copy.deepcopy(self.current_record_passing_vehicle)
        self.current_record_passing_vehicle = copy.deepcopy(self.last_waiting_moment)

    def calculate_decreasing_num(self):
        decreasing_num = 0.0
        for v_id in self.last_record_passing_vehicle.keys():
            if v_id in self.current_record_passing_vehicle:
                decreasing_num += 0
            else:
                decreasing_num += 1.0

        # print(decreasing_num)
        return decreasing_num
