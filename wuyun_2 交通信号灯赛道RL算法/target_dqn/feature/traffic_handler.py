...
class TrafficHandler:
    ...
    def update(self, env, obs, last_action=None):
        ...
        if need_to_predict:
            ...
            # 与环境交互, 执行动作, 获取下一步的状态
            self.logger.info(f"handler_last_action{last_action}")
            frame_no, obs, score, terminated, truncated, env_info = env.step(
                [last_action[0], last_action[1], last_action[2]]
            )
            self.predict_step += 1
            self.current_step += 1
            self.logger.info(f"handler_current_step:{self.current_step}")
            self.logger.info(f"handler_predict_step:{self.predict_step}")
        ...
        if need_to_predict:
            self.logger.info(f'handler_need_predict_old_phase:{self.old_phase}')
