...
def workflow(envs, agents, logger=None, monitor=None):
    ...
    for epoch in range(epoch_num):
        epoch_total_rew = 0
        epoch_total_delay_time = 0.0
        epoch_total_waited_time = 0.0
        epoch_total_waited_num = 0.0
        data_length = 0
        for g_data in run_episodes(episode_num_every_epoch, env, agent, g_data_truncat, usr_conf, logger):
            ...
            for data in g_data:
                ...
                total_delay_time.append(data.delay_time)
                total_waited_time.append(data.waited_time)
                total_waited_num.append(data.waited_num)


            total_rew = sum(total_rew)
            total_delay_time = sum(total_delay_time)
            total_waited_time = sum(total_waited_time)
            total_waited_num = sum(total_waited_num)

            epoch_total_rew += total_rew
            epoch_total_delay_time += total_delay_time
            epoch_total_waited_time += total_waited_time
            epoch_total_waited_num += total_waited_num

            agent.learn(g_data)
            g_data.clear()

        avg_step_reward = 0
        avg_step_delay_time = 0
        avg_step_waited_time = 0
        avg_step_waited_num = 0
        if data_length:
            avg_step_reward = f"{(epoch_total_rew/data_length):.2f}"
            avg_step_delay_time = epoch_total_delay_time/data_length
            avg_step_waited_time = epoch_total_waited_time/data_length
            avg_step_waited_num = epoch_total_waited_num/data_length
        ...
        if now - last_report_monitor_time > 60:
            ...
            monitor_data["diy_1"] = avg_step_delay_time
            monitor_data["diy_2"] = avg_step_waited_time
            monitor_data["diy_3"] = avg_step_waited_num

            if monitor:
                monitor.put_data({os.getpid(): monitor_data})
                last_report_monitor_time = now

        logger.info(f"Avg Step Reward: {avg_step_reward},delaytime:{avg_step_delay_time} Epoch: {epoch}, Data Length: {data_length}")
...
def run_episodes(n_episode, env, agent, g_data_truncat, usr_conf, logger):
    for episode in range(n_episode):
        ...
        while not done:
            ...
            logger.info(f'train_workflow_act:{act}')
            ...
            reward,delay_time,waited_time,waited_num = reward_shaping(_obs, act,traffic_handler,obs_data,_obs_data ,_env_info)
            # print('reward',reward)
