...
class Actor(nn.Module):
    ...
    def get_logprob(self,state,action):
        ...
        return logprob_a
class Normal_Actor(nn.Module):
    def __init__(self, state_size, action_size, neu_size):
        super(Normal_Actor, self).__init__()
    ...
    def get_act(self, state,deterministic,with_logprob):
        ...
        if deterministic:
            ...
            a = dist.rsample()
            if with_logprob:
                return a, dist.log_prob(a).sum(dim = 1,keepdim = True);
            else:
                return a,None
...
class SAC_ReplayBuffer():
    def __init__(self, state_size, action_size, num_envs,max_size,device):
        self.device = device
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        self.r[valid_index] = reward[:,np.newaxis]
        self.s_next[valid_index] = next_state
        self.dw[valid_index] = done[:,np.newaxis]
