...
class phi_net_decoder(nn.Module):
    ...
    def forward(self, s_abs,s_non_abs,s):
        state = torch.cat((s_abs,s_non_abs),1)
...
class abstraction_module():
    ...
    def train(self,replay_buffer,batch_size):
        ...
        loss = -torch.mean(log_prob_r) - torch.mean(log_prob_s) - torch.mean(log_prob_resconstruct)
    ...
    #
    #         loss = -torch.mean(log_prob_r) - torch.mean(log_prob_s)
    #         self.transition_net_optimizer.zero_grad();
    #         self.reward_net_optimizer.zero_grad();
    #         self.phi_net_optimizer.zero_grad();
    #         loss.backward();
